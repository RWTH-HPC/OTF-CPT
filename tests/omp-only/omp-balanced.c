//ALLOW_RETRIES: 5
//RUN: env OMP_NUM_THREADS=2 %load_otfcpt %t | %FileCheck --check-prefixes=CHECK %s
//RUN: env OMP_NUM_THREADS=4 %load_otfcpt %t | %FileCheck --check-prefixes=CHECK %s
//RUN: env OMP_NUM_THREADS=2 %load_otfcpt_omp %t | %FileCheck --check-prefixes=CHECK %s
//RUN: env OMP_NUM_THREADS=4 %load_otfcpt_omp %t | %FileCheck --check-prefixes=CHECK %s
...
//CHECK: Parallel Efficiency:                [[PE:0.99[0-9]+]]
